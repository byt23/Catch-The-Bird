...
class level8ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        hideTimer = Timer.scheduledTimer(timeInterval: 0.58, target: self, selector: #selector(hideBird), userInfo: nil, repeats: true)
        ...
        @objc func countDown(){
            ...
            if counter == 0 {
                ...
                if self.score >= 80 {
                    ...
                    if unlockedLevel < 9 {
                        UserDefaults.standard.set(9, forKey: "unlockedLevel")
                    }
                }
                ...
                if self.score >= 80 {
                    let level9 = UIAlertAction(title: "Next Level", style: .default) { _ in
                        self.performSegue(withIdentifier: "level9", sender: self)
                        }
                    alert.addAction(level9)
                    }
                ...
                let replayButton = UIAlertAction(title: "Try Again", style: .default) { _ in
                    ...
                    self.hideTimer = Timer.scheduledTimer(timeInterval: 0.58, target: self, selector: #selector(self.hideBird), userInfo: nil, repeats: true)
                    }

                let mainMenuButton = UIAlertAction(title: "Main Menu", style: .cancel) { _ in
                    self.performSegue(withIdentifier: "main8", sender: self)
                    }
