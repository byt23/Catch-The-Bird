...
class level11ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        hideTimer = Timer.scheduledTimer(timeInterval: 0.55, target: self, selector: #selector(hideBird), userInfo: nil, repeats: true)
        ...
        @objc func countDown(){
            ...
            if counter == 0 {
                ...
                if self.score >= 95 {
                    ...
                    if unlockedLevel < 12 {
                        UserDefaults.standard.set(12, forKey: "unlockedLevel")
                    }
                }
                ...
                if self.score >= 95 {
                    ...
                    let missingPoints = 95 - self.score
                        alertMessage = """
                    You need at least 95 points to reach the next level.
                ...
                if self.score >= 95 {
                    let level12 = UIAlertAction(title: "Next Level", style: .default) { _ in
                        self.performSegue(withIdentifier: "level12", sender: self)
                        }
                    alert.addAction(level12)
                    }
                ...
                let replayButton = UIAlertAction(title: "Try Again", style: .default) { _ in
                    ...
                    self.hideTimer = Timer.scheduledTimer(timeInterval: 0.55, target: self, selector: #selector(self.hideBird), userInfo: nil, repeats: true)
                    }

                let mainMenuButton = UIAlertAction(title: "Main Menu", style: .cancel) { _ in
                    self.performSegue(withIdentifier: "main11", sender: self)
                    }
