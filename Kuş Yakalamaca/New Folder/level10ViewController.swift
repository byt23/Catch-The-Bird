...
//  Kuş Yakalamaca
//
//  Created by BERKAY TURAN on 14.08.2025.
//
...
class level10ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        bird10.addGestureRecognizer(recognizer9)
        bird11.addGestureRecognizer(recognizer9)
        bird12.addGestureRecognizer(recognizer9)
        ...
        hideTimer = Timer.scheduledTimer(timeInterval: 0.65, target: self, selector: #selector(hideBird), userInfo: nil, repeats: true)
        ...
        @objc func countDown(){
            ...
            if counter == 0 {
                ...
                // Alert message based on score
                let alertMessage: String
                if self.score >= 60 {
                    alertMessage = "What a great game! You scored \(self.score) points."
                } else {
                    let missingPoints = 60 - self.score
                    alertMessage = """
                    You need at least 60 points to reach the next level.
                ...
                // Add "Next Level" button only if score >= 60
                if self.score >= 60 {
                    let level11 = UIAlertAction(title: "Next Level", style: .default) { _ in
                        self.performSegue(withIdentifier: "level11", sender: self)
                    }
                    alert.addAction(level11)
                }
                ...
                let mainMenuButton = UIAlertAction(title: "Main Menu", style: .cancel) { _ in
                    self.performSegue(withIdentifier: "main10", sender: self)
                }
    ...
    @IBAction func pauseButtonClicked(_ sender: Any) {
        ...
        let resumeAction = UIAlertAction(title: "Resume", style: .default) { _ in
            ...
            self.timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(self.countDown), userInfo: nil, repeats: true)
            self.hideTimer = Timer.scheduledTimer(timeInterval: 0.65, target: self, selector: #selector(self.hideBird), userInfo: nil, repeats: true)
        }
