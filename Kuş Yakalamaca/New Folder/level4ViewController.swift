...
class level4ViewController: UIViewController {
    ...
    hideTimer = Timer.scheduledTimer(timeInterval: 0.40, target: self, selector: #selector(hideBird), userInfo: nil, repeats: true)
        ...
        @objc func countDown(){
            ...
            if counter == 0 {
                ...
                if self.score >= 80 {
                    ...
                    let missingPoints = 80 - self.score
                        alertMessage = """
                    You need at least 80 points to reach the next level.
                ...
                // Add "Next Level" button only if score >= 80
                if self.score >= 60 {
                    let level5 = UIAlertAction(title: "Next Level", style: .default) { _ in
                        self.performSegue(withIdentifier: "level5", sender: self)
                        }
                    alert.addAction(level5)
                    }
                ...
                let replayButton = UIAlertAction(title: "Try Again", style: .default) { _ in
                    ...
                    self.hideTimer = Timer.scheduledTimer(timeInterval: 0.4, target: self, selector: #selector(self.hideBird), userInfo: nil, repeats: true)
                    }

                let mainMenuButton = UIAlertAction(title: "Main Menu", style: .cancel) { _ in
                    self.performSegue(withIdentifier: "main4", sender: self)
                    }
